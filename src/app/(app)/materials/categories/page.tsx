...
import { Plus, ListTree, X } from "lucide-react";
import { PageHeader } from "@/components/page-header";
import { EmptyState } from "@/components/empty-state";
import { ActiveBadge } from "@/components/badge";
import { useCompanyData } from "@/lib/use-company-data";
import { getMaterialCategories, createMaterialCategory } from "@/lib/client-queries";
...
export default function CategoriesPage() {
  ...
  const loadData = useCallback(async () => {
    if (!company) return;
    setDataLoading(true);
    const data = await getMaterialCategories(company.id);
    setCategories(data);
  ...
  return (
    <>
      <PageHeader
        ...
        description="Organize your materials by category"
        actions={
          canManage && (
            <button
      ...
      {categories.length === 0 ? (
        <div className="bg-card border border-border rounded-lg">
          <EmptyState
            ...
            title="No Categories"
            description="Add a category to organize your materials."
          />
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-border">
              {categories.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3"><ActiveBadge isActive={c.is_active} /></td>
                  <td className="px-5 py-3 text-sm text-muted">{new Date(c.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function AddCategoryModal({ companyId, onClose, onSuccess }: { companyId: string; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setPending(true);
    setError(null);
    if (!name.trim()) { setError("Category name is required."); setPending(false); return; }
    const result = await createMaterialCategory(companyId, name.trim());
    if (!result.success) { setError(result.error || "Failed to create category."); setPending(false); }
    else { onSuccess(); }
  };
        ...
        <div className="px-6 py-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="e.g. Cement" className="w-full rounded-md border border-border bg-white px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" disabled={pending} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed">{pending ? "Creating..." : "Create"}</button>
            </div>
          </form>
        </div>
      </div>
